...
if plan_file and fact_file:
    ...
    df_final = None # Инициализируем переменную
    ...
    if df_final is not None:
        ...
        # Создаем вкладки для лучшей организации интерфейса
        tab_dashboard, tab_pivot, tab_detail = st.tabs(["📈 Дашборд", "🛠️ Конструктор отчетов", "🔍 Детальный анализ и выгрузка"])

        # --- ВКЛАДКА 1: ДАШБОРД (ИДЕЯ 2) ---
        with tab_dashboard:
        ...
        # --- ВКЛАДКА 2: ДИНАМИЧЕСКАЯ СВОДНАЯ ТАБЛИЦА (ИДЕЯ 3) ---
        with tab_pivot:
            ...
            with col1:
                ...
                selected_grouping = st.multiselect(
                    "Выберите поля для группировки:", options=grouping_options, default=['Magazin', 'Segment']
                )
            with col2:
            ...
            if selected_grouping:
                if metric_type == 'Деньгах (ГРН)':
                    values_to_agg = {'Plan_GRN': 'sum', 'Fact_GRN': 'sum'}
                    plan_col, fact_col = 'Plan_GRN', 'Fact_GRN'
                else:
                    values_to_agg = {'Plan_STUKI': 'sum', 'Fact_STUKI': 'sum'}
                    plan_col, fact_col = 'Plan_STUKI', 'Fact_STUKI'
        ...
        # --- ВКЛАДКА 3: ДЕТАЛЬНЫЙ АНАЛИЗ (старый функционал в новой вкладке) ---
        with tab_detail:
