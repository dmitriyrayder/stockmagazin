...
st.set_page_config(page_title="Анализ по Сегментам v6.2", page_icon="🎯", layout="wide")
...
# --- Шаг 1: Загрузка файлов и Шаг 2: Настройка ---
...
if plan_file and fact_file:
    ...
    with st.form("processing_form"):
        ...
        for internal, display in FACT_REQUIRED_FIELDS.items():
            # Простой поиск по умолчанию для удобства
    ...
    if submitted:
        try:
            if plan_format == 'Широкий (горизонтальный)':
                ...
                # Здесь можно добавить маппинг, если нужно. Пока предполагаем правильные имена.
            ...
            plan_cols_for_merge = [key for key in list(plan_df.columns) if key not in fact_cols_to_merge] + merge_keys
...
if st.session_state.processed_df is not None:
    ...
    if not problem_stores_df.empty:
        with st.expander("Показать детализацию по сегментам для проблемных магазинов"):
            ...
            segment_summary['Отклонение_%_шт'] = np.where(segment_summary['Plan_STUKI'] > 0, (segment_summary['Отклонение_шт'] / segment_summary['Plan_STUKI']) * 100, np.inf if segment_summary['Отклонение_шт'] != 0 else 0)
    ...
    # Важно: `problem_stores_df` может быть пустой, обрабатываем это
