...
if plan_file and fact_file:
    ...
    with st.form("processing_form"):
        ...
        for internal, display in FACT_REQUIRED_FIELDS.items():
            default_selection = next((col for col in fact_cols if display.lower() in col.lower()), fact_cols[0])
            fact_mappings[internal] = st.selectbox(f'"{display}"', fact_cols, index=fact_cols.index(default_selection), key=f'fact_{internal}')
        
        col1, col2 = st.columns(2)
        with col1:
            remove_duplicates = st.checkbox("Удалить дубликаты по ключам", value=True)
        with col2:
            fill_zeros = st.checkbox("Заполнить пропуски в числовых данных нулями", value=True)
    ...
    if submitted:
        try:
            ...
            if plan_df is None:
                st.error("Не удалось обработать файл 'План'. Проверьте структуру файла.")
                st.stop()
...
if st.session_state.processed_df is not None:
    ...
    if not stores_for_selection:
        ...
        if 'Segment' in filtered_df.columns and filtered_df['Segment'].nunique() > 1:
            ...
            st.dataframe(display_table, column_config={
                ...
                "Отклонение, п.п.": st.column_config.NumberColumn("Отклонение, п.п.", help="Разница долей (Факт % - План %)", format="%+,.1f п.п.")
    ...
    st.download_button(
        ...
        file_name=f"полный_анализ_{datetime.now().strftime('%Y%m%d_%H%M')}.xlsx",
